...
def oleg_func_linear(file, user_key,
    ...
    for i in range(1, 5):
        exec(f"df = next_month_prediction(df, model, "
             f"out_of_stock=out_of_stock{i}, promo=promo{i}, epidemic=epidemic{i})")
    ...
    return [result_for_client_month,
            result_for_client_allval,
            f'user_{user_key}_lr_real.png',
            f'user_{user_key}_final.png']
